...
class UserRepository(UserRepositoryInterface):
    def __init__(self):
        self.columns = ["id", "name", "email", "password", "account_type", "created_at", "updated_at"]
    ...
    def insert_one(self, user: UserModel) -> UserModel:
        ...
        result = cursor.execute(
            ...
            (user_to_insert["name"], user_to_insert["email"], pass_hash.decode(),
            user_to_insert["account_type"], user_to_insert["created_at"], user_to_insert["updated_at"])
        )
        conn.commit()
    ...
    def find_by_email(self, email: str) -> UserModel | None:
        cursor.execute(f"select * from {UserModel.Meta.db_name} where email = %s", (email,))
