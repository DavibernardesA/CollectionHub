...
from src.domain.core.ports.repositories.flf_item_repository_interface import (
    FLFItemRepositoryInterface,
)


class FLFCollectionRepository(FLFItemRepositoryInterface):
    def __init__(self):
        self.columns = ["account_id", "action", "item_id"]

    def find_by_id_and_action(self, account_id, action):
        cursor = get_cursor()

        query = (
            f"select * from {FLFItem.Meta.db_name} where account_id = %s and action = %s"
        )
    ...
    def delete(self, account_id, action):
        ...
        query = (
            f"delete from {FLFItem.Meta.db_name} where account_id = %s and action = %s "
        )
